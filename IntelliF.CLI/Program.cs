...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        // Шаг 1: Системное сообщение (ИСПРАВЛЕНО)
        var systemMsg = "🚀 Ты — F# эксперт. Весь код в ```fsharp...``` будет выполнен в FSI. " +
                        ...
                        "После определения функции обязательно добавь вызов функции с тестовыми данными, например: sumOfSquares 5;;";
        ...
        // Шаг 2: Отправка задания (ИСПРАВЛЕНО - добавлено требование вызова)
        var taskMsg = "📝 Задание: Напиши F#-функцию, вычисляющую сумму квадратов чисел от 1 до n. " +
                      "После определения функции добавь вызов функции с тестовым значением (например, для n=5). ";
        ...
        while (attempt < maxAttempts && !success)
        {
            ...
            if (!hasErrors)
            {
                ...
                // ВЫВОД РЕЗУЛЬТАТОВ (НОВОЕ) 🚨
                Console.WriteLine("📋 Результаты выполнения:");
                for (int i = 0; i < results.Count; i++)
                {
                    if (!string.IsNullOrEmpty(results[i].stdout))
                    {
                        Console.WriteLine($"📦 Блок #{i + 1} результат: {results[i].stdout}");
                    }
                }
                Console.WriteLine("══════════════════════");
            ...
            resultPrompt.AppendLine("🚫 Исправь код и отправь исправленную версию с примером вызова в блоке fsharp.");
